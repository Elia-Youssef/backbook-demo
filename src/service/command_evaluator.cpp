...
[[nodiscard]] CommandEvaluationError evaluation_error(
    const CommandEvaluationErrorCode code) {
    return CommandEvaluationError{code};
}

[[nodiscard]] CommandEvaluationError domain_rejection(
    domain::StateError state_error) {
    auto failure =
        evaluation_error(CommandEvaluationErrorCode::DomainRejected);
...
[[nodiscard]] ResultOutcome successful_result(Result result) {
    return ResultOutcome::success(
        journal::CommandResult{std::move(result)});
}
...
[[nodiscard]] EvaluationOutcome complete_transition(
    ...
    if (!invariants_hold(prospective.value())) {
        return EvaluationOutcome::failure(
            evaluation_error(
                CommandEvaluationErrorCode::InvariantViolation));
    }
    ...
    return EvaluationOutcome::success(
        CommandEvaluation{std::move(prospective).value(),
                          std::move(event),
        ...
        [&current](const auto& value) -> EvaluationOutcome {
            ...
            if constexpr (std::is_same_v<Value, BookTradeCommand>) {
                ...
                return complete_transition(
                    ...
                    [&value](const domain::State& after) {
                        return successful_result(
                            journal::TradeBookedResult{
                                value.trade_id, 1U, after.version()});
                    });
            } else if constexpr (
                std::is_same_v<Value, ConfirmTradeCommand>) {
                ...
                return complete_transition(
                    ...
                    [&value](const domain::State& after) {
                        return successful_result(
                            journal::TradeConfirmedResult{
                                value.trade_id,
                                value.expected_version,
                                after.version()});
                    });
            } else if constexpr (
                std::is_same_v<Value, AmendTradeCommand>) {
                ...
                std::is_same_v<Value, CancelTradeCommand>) {
                auto prospective = domain::cancel_trade(
                    current,
                    value.trade_id,
                    value.expected_version,
                    value.reversal_ids);
                ...
                auto prospective =
                    domain::run_eod(current, value.as_of_date);
                return complete_transition(
                    std::move(prospective),
                    journal::Event{
                        journal::EodRunEvent{value.as_of_date}},
                    [&current, &value](const domain::State& after) {
                        const auto before_count =
                            settled_trade_count(current);
                        const auto after_count =
